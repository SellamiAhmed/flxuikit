...
<Callout type="warning">
  If you're porting examples from the tidbcloud reference docs, several prop names don't carry over — this
  package's `SearchArea` uses different names for the same concepts, and drops one prop entirely:

| tidbcloud     | This package                                                                                                                                                                          |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `filters`     | `data`                                                                                                                                                                                |
| `persistUrl`  | `recoverFromURLEnabled`                                                                                                                                                               |
| `urlStateKey` | `formStateQueryKey`                                                                                                                                                                   |
| `clearText`   | `clearFiltersText`                                                                                                                                                                    |
| `debug`       | `debugEnabled` (present in the type, but not wired up — see below)                                                                                                                    |
| `formProps`   | **not supported** — `SearchArea` builds its own `useForm()`/`FormProvider` directly rather than rendering the `Form` component, so there's nothing to forward extra `Form` props into |
